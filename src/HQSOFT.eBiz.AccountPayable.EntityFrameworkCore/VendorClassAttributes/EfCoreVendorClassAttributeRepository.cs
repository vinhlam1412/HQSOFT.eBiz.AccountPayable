...
namespace HQSOFT.eBiz.AccountPayable.VendorClassAttributes
{
    public class EfCoreVendorClassAttributeRepository : EfCoreRepository<AccountPayableDbContext, VendorClassAttribute, Guid>, IVendorClassAttributeRepository
    {
        ...
        {
            return query
                .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.VendorClassAttribute.IdAttribute.ToLower().Contains(filterText.ToLower()) || e.VendorClassAttribute.ControlType.ToLower().Contains(filterText.ToLower()) || e.VendorClassAttribute.DefaultValue.ToLower().Contains(filterText.ToLower()))
                    .WhereIf(isActive.HasValue, e => e.VendorClassAttribute.IsActive == isActive)
                    .WhereIf(!string.IsNullOrWhiteSpace(idAttribute), e => e.VendorClassAttribute.IdAttribute.ToLower().Contains(idAttribute.ToLower()))
        ...
        public async Task<List<VendorClassAttribute>> GetListAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = ApplyFilter((await GetQueryableAsync()), filterText, isActive, idAttribute, sortOrderMin, sortOrderMax, isRequired, isInternal, controlType, defaultValue, idxMin, idxMax);
        ...
        {
            return query
                    .WhereIf(!string.IsNullOrWhiteSpace(filterText), e => e.IdAttribute.ToLower().Contains(filterText.ToLower()) || e.ControlType.ToLower().Contains(filterText.ToLower()) || e.DefaultValue.ToLower().Contains(filterText.ToLower()))
                    .WhereIf(isActive.HasValue, e => e.IsActive == isActive)
                    .WhereIf(!string.IsNullOrWhiteSpace(idAttribute), e => e.IdAttribute.ToLower().Contains(idAttribute.ToLower()))
