...
namespace HQSOFT.eBiz.AccountPayable.VendorClassAttributes
{
    ...
    public class VendorClassAttributesAppService : ApplicationService, IVendorClassAttributesAppService
    {
        ...
        public virtual async Task<PagedResultDto<VendorClassAttributeWithNavigationPropertiesDto>> GetListAsync(GetVendorClassAttributesInput input)
        {
            ...
            var items = await _vendorClassAttributeRepository.GetListWithNavigationPropertiesAsync(input.FilterText, input.IsActive, input.IdAttribute, input.SortOrderMin, input.SortOrderMax, input.IsRequired, input.IsInternal, input.ControlType, input.DefaultValue, input.IdxMin, input.IdxMax, input.VendorClassId, input.Sorting, input.MaxResultCount, input.SkipCount);

            return new PagedResultDto<VendorClassAttributeWithNavigationPropertiesDto>
            {
                TotalCount = totalCount,
                Items = ObjectMapper.Map<List<VendorClassAttributeWithNavigationProperties>, List<VendorClassAttributeWithNavigationPropertiesDto>>(items)
            };
        }
