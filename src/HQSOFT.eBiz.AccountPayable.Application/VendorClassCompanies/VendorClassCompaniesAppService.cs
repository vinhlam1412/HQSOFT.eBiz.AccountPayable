...
namespace HQSOFT.eBiz.AccountPayable.VendorClassCompanies
{
    ...
    public class VendorClassCompaniesAppService : ApplicationService, IVendorClassCompaniesAppService
    {
        ...
        public virtual async Task<PagedResultDto<VendorClassCompanyWithNavigationPropertiesDto>> GetListAsync(GetVendorClassCompaniesInput input)
        {
            ...
            var items = await _vendorClassCompanyRepository.GetListWithNavigationPropertiesAsync(input.FilterText, input.CompanyId, input.Exclude, input.IdxMin, input.IdxMax, input.VendorClassId, input.Sorting, input.MaxResultCount, input.SkipCount);

            return new PagedResultDto<VendorClassCompanyWithNavigationPropertiesDto>
            {
                TotalCount = totalCount,
                Items = ObjectMapper.Map<List<VendorClassCompanyWithNavigationProperties>, List<VendorClassCompanyWithNavigationPropertiesDto>>(items)
            };
        }
